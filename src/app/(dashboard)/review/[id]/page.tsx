...
const STATUS_STYLES: Record<QueueStatus, { label: string; className: string }> = {
  ...
  IN_REVIEW: { label: "In Review", className: "bg-primary-50 text-primary border border-primary-200" },
  DECIDED: { label: "Decided", className: "bg-default-100 text-default-500 border border-default-200" },
};
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-default-500 mt-1 ml-11">
            Case{" "}
            <span className="font-mono text-xs bg-default-100 px-1.5 py-0.5 rounded">{queue.queue_id}</span>
            {" "}· Matched against{" "}
        ...
        <div className={`shrink-0 flex items-center gap-2 rounded-xl border px-3 py-2 ${
          ...
          <span className={`text-xl font-bold tabular-nums ${
            queue.ai_confidence >= 80 ? "text-danger" : queue.ai_confidence >= 60 ? "text-warning" : "text-success"
          }`}>
            {queue.ai_confidence}%
          </span>
